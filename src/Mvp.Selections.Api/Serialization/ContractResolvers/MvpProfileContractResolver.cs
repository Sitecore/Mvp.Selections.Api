...
public class MvpProfileContractResolver : CamelCasePropertyNamesContractResolver
{
    ...
    private readonly string[] _userExcludedMembers = [nameof(User.Consents), nameof(User.Applications), nameof(User.Mentors), nameof(User.Reviews), nameof(User.Email), nameof(User.Roles), nameof(User.Rights), nameof(User.IsMentor), nameof(User.IsOpenToNewMentees), nameof(User.MentorDescription)];
