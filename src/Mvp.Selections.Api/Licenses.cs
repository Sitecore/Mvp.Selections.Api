...
namespace Mvp.Selections.Api
{
            ...
            ILicenseZipParser licenseZipParser) : Base<Licenses>(logger, serializer, authService)
    {
            ...
            Guid id)
        {
            ...
            {
                License? licenseUpdate = await Serializer.DeserializeAsync<License>(req.Body);

                OperationResult<License> result = licenseUpdate != null
                    ? await licenseService.UpdateAsync(licenseUpdate, id)
        ...
        public Task<IActionResult> Download(
            [HttpTrigger(AuthorizationLevel.Anonymous, GetMethod, Route = "v1/users/current/license/download")]
            HttpRequest req)
        {
